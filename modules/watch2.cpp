...
private:
	...
	void Watch(const CString& sHostMask, const CString& sTarget, const CString& sPattern, bool bNotice = false) {
		...
		} else if (!sHostMask.size()) {
			sMessage = "Watch2: Not enough arguments.  Try Help";
		} else if (rHostMask.error().size() != 0) {
			sMessage = CString("Watch2: Invalid regular expression for hostname: " + rHostMask.error());
		} else if (rPattern.error().size() != 0) {
			sMessage = CString("Watch2: Invalid regular expression for pattern: " + rPattern.error());
		}
	...
	bool ConfigPage(CWebSock& WebSock, CTemplate& Tmpl) {
		...
		for (list<CWatchEntry>::iterator it = m_lsWatchers.begin(); it != m_lsWatchers.end(); ++it, uIdx++) {
			...
			Row["DetachedClientOnly"] = CString(WatchEntry.IsDetachedClientOnly());
			Row["DetachedChannelOnly"] = CString(WatchEntry.IsDetachedChannelOnly());
		}
	...
	bool ImportExportPage(CWebSock& WebSock, CTemplate& Tmpl) {
		...
		for (list<CWatchEntry>::iterator it = m_lsWatchers.begin(); it != m_lsWatchers.end(); ++it, uIdx++, sIdx = CString(uIdx)) {
			...
			if (WatchEntry.IsDetachedClientOnly()) {
				Tmpl.AddRow("ImportExportLoop")["ImportExport"] = "ENABLEDETACHEDCLIENTONLY " + sIdx;
			}

			if (WatchEntry.IsDetachedChannelOnly()) {
				Tmpl.AddRow("ImportExportLoop")["ImportExport"] = "ENABLEDETACHEDCHANNELONLY " + sIdx;
			}
		}
